(() => {
  ...
  function bindNightChuteJump() {
    main.querySelectorAll('[data-night-chute-jump]').forEach((button) => button.addEventListener('click', () => {
      const target = main.querySelector(`[data-night-chute-section="${button.dataset.nightChuteJump}"]`);
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.042';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.042.js?v=0.042',
        { updateViaCache: 'none' }
      );
