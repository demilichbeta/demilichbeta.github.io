(function (root, factory) {
  ...
  // 0.053：特殊盤點只修改既有正式分類，不建立第二份數量資料。
  ...
  function migrateShift(shift) {
    ...
    // 0.053：特殊盤點只保存「確認狀態／簽章」，數量仍全部來自 events。
  ...
  function stationStats(count, carrier = 'ALL') {
    ...
    // 0.053：online一按＋1即先列入現場應有；轉完成時 online→secondary，因此應有總數不跳動。
  ...
  function convertOnlineToSecondary(shift, station, carrier = 'ALL') {
    ...
    targets.forEach((item) => {
      // 0.053：二分「轉完成」代表6點後現場已確認此載具；online→secondary讓應有不重複增加，actual同步增加。
