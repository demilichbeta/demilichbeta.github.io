(() => {
  ...
  function renderReturns() {
    ...
    main.innerHTML = `
      ...
      <section class="card return-copy-card"><button id="copyReturnReportBtn" class="primary-btn full-width" type="button">複製回倉回報文字</button><p class="small-note">上半部保留00後各時段紀錄；00後合計下方以固定欄寬表格列出「00前＋00後」完整回倉總數，0也會顯示。</p></section>`;
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.063';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.063.js?v=0.063',
        { updateViaCache: 'none' }
      );
