(() => {
  ...
  const VIEW_TITLES = {
    ...
    morning: '早班數量', transit: '過境貨', loaded: '提前載走', reports: '03:00 快速回報',
  ...
  function defaultState() {
    return {
      ...
      activeView: 'night',
      ui: {
        statsGroup: 'ALL', statsAnomaliesOnly: false, statsCarrier: 'ALL',
  ...
  function renderMorning() {
    const shift = requireShift(); if (!shift) return;
    const counts = L.computeCounts(shift);
    main.innerHTML = `<section class="card"><strong>00:00 早班盤點</strong><p class="small-note">依字母系列排列；籠車、棧板分開輸入。</p></section>
  ...
  function renderOnline() {
    ...
    main.innerHTML = `
      <section class="card"><strong>04:30 線上未退出</strong><p class="small-note">全部＋1依各站預設載具；個別例外可啟用下一筆非預設。綠框代表已清零，橘框代表尚待轉完成。</p>
        <div class="bulk-online-actions"><button class="primary-btn" data-online-all-add>全部 ${L.REPORT_GROUPS.FIVE_AM.length} 站＋1</button><button class="secondary-btn" data-convert-all-online>全部線上轉完成</button></div>
      </section>
      ...
      <div class="input-list">${L.STATIONS.map((station) => {
    ...
    main.querySelector('[data-online-all-add]').addEventListener('click', async () => {
      const result = L.addOnlineToAllStations(shift, 1); await saveState(); vibrate(80); renderCurrentView(); showToast(`全部${result.stations}站已依預設載具＋1`);
    });
  ...
  function renderInventory() {
    ...
    main.innerHTML = `<section class="card"><strong>現場盤點</strong><p class="small-note">籠車、棧板分別輸入；右側即時顯示應有與總差異。</p></section>
      <div class="inventory-list">${L.STATIONS.map((station) => {
        const s = allStats[station]; const cls = s.difference === 0 ? 'difference-good' : 'difference-bad';
        return `<div class="inventory-card ${cls}" data-inventory-row="${station}"><div class="inventory-head"><strong>${station}</strong><span>應${s.expected}｜差${s.difference}</span></div>
  ...
  function renderStats() {
    ...
    const filtered = L.STATIONS.filter((station) => (group === 'ALL' || station.startsWith(group)) && (!anomaliesOnly || stats[station].difference !== 0));
    main.innerHTML = `
      ...
      <div>${filtered.length ? filtered.map((station) => statsCard(station, stats[station], allStats[station], cageStats[station], palletStats[station], carrier)).join('') : '<div class="empty-state">沒有符合條件的站所。</div>'}</div>`;
  ...
  function statsCard(station, s, all, cage, pallet, carrier) {
    ...
    const fields = [['早班', s.morning], ['夜班', s.night], ['過境', s.transit], ['線上', s.online], ['回報', s.reportTotal], ['載走', s.loaded], ['應有', s.expected], ['現場', s.actual], ['差異', s.difference]];
  ...
  function renderReports() {
    ...
    main.innerHTML = `<section class="card"><strong>03:00 派車快速回報</strong><p class="small-note">CS／SS／KS；總數下方保留籠車／棧板細分。</p></section>
      ...
      <div class="report-list">${L.REPORT_GROUPS.THREE_AM.map((station) => { const s = stats[station]; return `<div class="report-row"><strong>${station}</strong><span><small>早</small>${s.morning}</span><span><small>夜</small>${s.night}</span><span><small>過</small>${s.transit}</span><span class="report-total"><small>總</small>${s.reportTotal}<em>籠${cage[station].reportTotal}／板${pallet[station].reportTotal}</em></span></div>`; }).join('')}</div>`;
  ...
  function returnBucketHtml(bucket) {
    ...
    return `<article class="return-bucket"><h3>${bucket.label}</h3><p>合計${bucket.total}｜籠${bucket.cage}｜板${bucket.pallet}</p>${details}</article>`;
  }
  ...
  function exportAllJSON() { const payload = { app: '物流夜班點貨', schemaVersion: 5, exportedAt: new Date().toISOString(), state }; downloadFile(`夜班點貨_完整備份_${L.localDate()}.json`, JSON.stringify(payload, null, 2), 'application/json'); showToast('JSON備份已匯出'); }
