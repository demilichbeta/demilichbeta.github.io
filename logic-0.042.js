(function (root, factory) {
  ...
  const CHUTES = [
    ...
    { id: 8, name: '第八滑道', stations: ['CT2', 'CS12', 'NT10', 'NT11', 'NT12', 'NT13'] },
    { id: 9, name: '第九滑道', stations: ['CS4', 'CT5', 'NS15', 'NT16', 'NT17', 'NS18', 'NT19'] },
    { id: 10, name: '第十滑道', stations: ['CS3', 'CT6', 'NT20', 'NT21', 'NT22', 'NT23', 'YT1', 'HT1'] },
  ...
  const STATION_GROUPS = {
    ...
    E: [...SERIES_GROUPS.ET, ...SERIES_GROUPS.HT, ...SERIES_GROUPS.YT].sort(operationalStationSort),
  };
  ...
  const RETURN_REPORT_SOURCE_ORDER = ['DC2', 'DC4', 'DC9', 'DC11', 'NS1', 'NS2', 'NS5', 'SDC', 'DC12', 'CS4'];
  ...
  function migrateShift(shift) {
    ...
    shift.preMidnightTransferCage = Math.max(
      0,
      Math.floor(Number(shift.preMidnightTransferCage || 0))
    );
    ...
    shift.schemaVersion = 14;
    recomputeEventAfters(shift);
    return shift;
  }

  function getPreMidnightTransferCage(shift) {
    migrateShift(shift);
    return shift.preMidnightTransferCage;
  }

  function setPreMidnightTransferCage(shift, value) {
    migrateShift(shift);
    shift.preMidnightTransferCage = Math.max(
      0,
  ...
  function adjustPreMidnightTransferCage(shift, delta) {
    return setPreMidnightTransferCage(
      shift,
      getPreMidnightTransferCage(shift) + Number(delta || 0)
    );
  }
  ...
  function makeReturnReportText(shift) {
    ...
    const hasCounts = buckets.some((bucket) => bucket.total > 0);
    const hasNotes = buckets.some((bucket) => (bucket.notes || []).length > 0);
    if (!hasCounts && !hasNotes) return '目前無回倉紀錄';
    ...
    const lines = sourceOrder.map((source) => {
      const entries = buckets
        .map((bucket) => ({
          bucket,
          value: bucket.sources?.[source],
        }))
        .filter(({ value }) => Number(value?.total || 0) > 0)
        .map(({ bucket, value }) =>
          `${quantityText(value)} ${bucketStartTime(bucket)}`
        );
    ...
    const totals = computeReturnCounts(shift).carrierTotals;
    ...
    if (notes.length) {
      ...
      lines.push('備註');
      notes.forEach((note) => {
        lines.push(`${note.time} ${note.text}`);
      });
    }
  ...
  function computeStatsOverview(shift) {
    ...
    const overview = STATIONS.reduce((result, station) => {
      ...
      return result;
    }, { morningNightCage: 0, morningNightPallet: 0, transitTotal: 0 });

    // 00前已轉出的籠車沒有站所明細，只加入統計總覽的中＋夜籠數。
    overview.morningNightCage += getPreMidnightTransferCage(shift);
    return overview;
  }
  ...
  function makeWorkLogText(shift) {
    ...
    lines.push(`回倉合計：${returns.carrierTotals.total}（${returns.carrierTotals.cage}籠／${returns.carrierTotals.pallet}板）`);
    lines.push('', '站所統計：');
    STATIONS.forEach((station) => {
  ...
  return {
    VERSION, CHUTES, STATIONS, STATION_RENAMES, SERIES_PREFIX_ALIASES, SERIES_ORDER, SERIES_GROUPS, GROUP_ORDER, STATION_GROUPS, REPORT_GROUPS, ONLINE_BULK_STATIONS, RETURN_SOURCES, RETURN_REPORT_SOURCE_ORDER,
    ...
    createShift, migrateShift, getPreMidnightTransferCage, setPreMidnightTransferCage, adjustPreMidnightTransferCage,
