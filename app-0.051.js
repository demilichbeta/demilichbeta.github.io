(() => {
  ...
  // 0.051：特殊盤點未確認前只存在記憶體草稿；不寫入第二份正式數量。
  ...
  // 0.051：特殊盤點是既有中班／夜班／過境／現場資料的高權限確認入口。
  ...
  function specialStationHtml(shift, station, counts, cageStats, palletStats) {
    ...
    const secondaryNote = preview.secondary > 0
      ? `<small class="special-secondary-note">二分完成 ${preview.secondary}${carrierShort(carrier)} 已包含在來源／應有</small>`
      : '';
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.051';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.051.js?v=0.051',
        { updateViaCache: 'none' }
      );
