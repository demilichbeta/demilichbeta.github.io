(() => {
  ...
  function renderNight() {
    ...
    main.innerHTML = `
      <section class="card night-mode-card compact-mode-card"><div><strong>${correction ? '夜班載具數修正模式' : '蓋章後，按站所一次。'}</strong>
        <details class="inline-guide"><summary>操作說明</summary><p class="small-note">${correction ? '籠車與棧板可分別調整。' : '正常依站所預設載具；偶發另一種載具時，先啟用「下一筆非預設」。'}</p></details></div>
  ...
  function renderLoaded() {
    ...
    main.innerHTML = `
      ${guideDetailsHtml('提前載走', '先選系列，再直接增減站所', '一次只展開N／T／C／S／E其中一個系列；再次按同一系列可收合。上方已載走小卡可直接定位站所。正常採預設載具，偶發另一種載具時先啟用下一筆非預設。')}
  ...
  function renderMorning() {
    ...
    const preMidnightTransferCage = L.getPreMidnightTransferCage(shift);

    main.innerHTML = `${guideDetailsHtml('00:00 中班盤點', '預設載具快速輸入', '依 NS／Nt、TS／Tt、CS／Ct、S、E 分組。正常採站所預設載具；按＋／−後可選1、2、5，點中央數字可直接輸入。')}
      <button id="copyMorningReportBtn" class="secondary-btn full-width morning-copy-btn">複製中班各站所籠／板數</button>
      ...
      <section class="card pre-midnight-transfer-card">
        <div class="pre-midnight-transfer-head">
          <div>
            <strong>00前轉出總數</strong>
            <small>只計籠車；加到統計核對的中＋夜籠數，不列入站所複製文字。</small>
          </div>
          <span>籠</span>
        </div>
        <div class="pre-midnight-transfer-control">
          <button type="button" data-pre-midnight-change="-1" aria-label="00前轉出籠數減1">−1</button>
          <input id="preMidnightTransferInput" type="number" min="0" step="1" inputmode="numeric" value="${preMidnightTransferCage}" aria-label="00前轉出總籠數">
          <button type="button" data-pre-midnight-change="1" aria-label="00前轉出籠數加1">＋1</button>
        </div>
      </section>`;
    ...
    main.querySelectorAll('[data-pre-midnight-change]').forEach((button) =>
      button.addEventListener('click', async () => {
        const value = L.adjustPreMidnightTransferCage(
          shift,
          Number(button.dataset.preMidnightChange)
        );
        await saveState();
        vibrate();
        showToast(`00前轉出已設定為${value}籠`);
        renderMorning();
        updateHeader();
      })
    );

    el('preMidnightTransferInput').addEventListener('change', async (event) => {
      const value = L.setPreMidnightTransferCage(
        shift,
  ...
  function renderOnline() {
    ...
    main.innerHTML = `
      ${guideDetailsHtml('二次分理', '轉完成才計算數量', '<b>轉完成才計算數量。</b>尚未轉完成的數字只代表待處理，不會進入統計核對或快速回報。')}
      <section class="card compact-action-card"><div class="bulk-online-actions"><button class="primary-btn" data-online-all-add>全部 NS／Nt／TS／Tt ${L.ONLINE_BULK_STATIONS.length} 站＋1</button><button class="secondary-btn" data-convert-all-online>全部待處理轉完成</button></div></section>
      ...
      ${L.CHUTES.map((chute) => `<section class="online-chute-section"><h2 class="online-chute-title">${chute.name}</h2><div class="input-list">${chute.stations.map((station) => {
        ...
        return `<div class="online-row ${zero ? 'online-zero' : 'online-pending'}"><strong>${stationLabel(station)}</strong><button class="online-adjust minus" data-online-change="-1" data-station="${station}">−1</button>
          <span class="online-count"><b>${total}</b><small>${carrierPair(pending.cage, pending.pallet)}</small></span><button class="online-adjust plus" data-online-change="1" data-station="${station}">＋1</button>
    ...
    main.querySelector('[data-online-all-add]').addEventListener('click', async () => {
      const result = L.addOnlineToAllStations(shift, 1); await saveState(); vibrate(80); renderCurrentView(); showToast(`NS／Nt／TS／Tt 共${result.stations}站待處理已依預設載具＋1`);
    });
  ...
  function renderInventory() {
    ...
    main.innerHTML = `${guideDetailsHtml('現場盤點', '預設載具優先，顏色提示核對結果', '預設只顯示各站常用載具；需要另一種載具時按「＋ 非預設」。卡片上方顯示中班、過境及盤點總應有籠板數；總數已扣除提前載走，但不另外顯示載走。兩種載具都正確才顯示綠框，任一不符即顯示紅框。')}
  ...
  function renderStats() {
    ...
    main.innerHTML = `
      ...
      <section class="stats-overview-section">
        ...
        <p class="small-note stats-overview-note">中＋夜不含過境、不扣除已載走，籠數另含00前轉出總數。</p>
      </section>
  ...
  function exportAllJSON() { const payload = { app: '物流夜班點貨', schemaVersion: 14, exportedAt: new Date().toISOString(), state }; downloadFile(`夜班點貨_完整備份_${L.localDate()}.json`, JSON.stringify(payload, null, 2), 'application/json'); showToast('JSON備份已匯出'); }
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.041';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.041.js?v=0.041',
        { updateViaCache: 'none' }
      );
