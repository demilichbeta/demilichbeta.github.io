(function (root, factory) {
  ...
  // 0.064：當班臨時回倉站所。固定來源維持原清單，特殊站所只存在當班並排在固定來源後方。
  const RETURN_RESERVED_CUSTOM_SOURCES = new Set(['NS']);

  function normalizeReturnSourceName(source) {
    return String(source || '').trim().toUpperCase().replace(/\s+/g, '');
  }

  function isValidCustomReturnSourceName(source) {
    return /^[A-Z0-9_-]{2,12}$/.test(normalizeReturnSourceName(source));
  }

  function returnSourcesForShift(shift) {
    const custom = Array.isArray(shift?.returnCustomSources) ? shift.returnCustomSources : [];
    return [...RETURN_SOURCES, ...custom.filter((source) => !RETURN_SOURCES.includes(source))];
  }
  ...
  function migrateShift(shift) {
    ...
    // 0.064：特殊回倉站所只保存當班來源名稱，不建立另一套回倉數量。
    // 舊資料若已有非固定 returnEvents 來源，也會在下方事件正規化後自動補入此名單。
    const migratedReturnCustomSources = [];
    (Array.isArray(shift.returnCustomSources) ? shift.returnCustomSources : []).forEach((rawSource) => {
      const source = normalizeReturnSourceName(rawSource);
      if (!isValidCustomReturnSourceName(source)) return;
      if (RETURN_SOURCES.includes(source) || RETURN_RESERVED_CUSTOM_SOURCES.has(source)) return;
      if (!migratedReturnCustomSources.includes(source)) migratedReturnCustomSources.push(source);
    });
    shift.returnCustomSources = migratedReturnCustomSources;

    // 0.064：00前回倉獨立保存，不混入00後30分鐘 returnEvents。
    ...
    Object.entries(sourcePreMidnightReturns.counts || {}).forEach(([rawSource, rawCounts]) => {
      const source = normalizeReturnSourceName(rawSource);
      if (!RETURN_SOURCES.includes(source) && !shift.returnCustomSources.includes(source)) return;
      const cage = Math.max(0, Math.floor(Number(rawCounts?.cage || 0)));
      const pallet = Math.max(0, Math.floor(Number(rawCounts?.pallet || 0)));
    ...
    // 保留舊資料中曾經存在的非固定回倉來源，升級後自動視為當班特殊站所。
    shift.returnEvents.forEach((event) => {
      const source = normalizeReturnSourceName(event.source);
      if (!isValidCustomReturnSourceName(source)) return;
      if (RETURN_SOURCES.includes(source) || RETURN_RESERVED_CUSTOM_SOURCES.has(source)) return;
      if (!shift.returnCustomSources.includes(source)) shift.returnCustomSources.push(source);
    });
  ...
  function preMidnightReturnSourceCounts(shift, source) {
    migrateShift(shift);
    const cleanSource = normalizeReturnSourceName(source);
    if (!returnSourcesForShift(shift).includes(cleanSource)) throw new Error('未知回倉來源');
  ...
  function computePreMidnightReturnCounts(shift) {
    migrateShift(shift);
    const bySource = {};
    returnSourcesForShift(shift).forEach((source) => {
  ...
  function addCustomReturnSource(shift, source) {
    migrateShift(shift);
    const cleanSource = normalizeReturnSourceName(source);
    if (!cleanSource) throw new Error('請輸入特殊站所代碼');
    if (!isValidCustomReturnSourceName(cleanSource)) throw new Error('站所代碼請使用2–12碼英文字母、數字、- 或 _');
    if (RETURN_RESERVED_CUSTOM_SOURCES.has(cleanSource)) throw new Error(`${cleanSource} 為回倉彙總保留名稱，不能新增`);
    if (RETURN_SOURCES.includes(cleanSource)) throw new Error(`${cleanSource} 已是固定回倉站所`);
    if (shift.returnCustomSources.includes(cleanSource)) throw new Error(`${cleanSource} 已經新增`);
    shift.returnCustomSources.push(cleanSource);
    return cleanSource;
  }

  function customReturnSourceTotals(shift, source) {
    migrateShift(shift);
    const cleanSource = normalizeReturnSourceName(source);
    const pre = shift.preMidnightReturns.counts[cleanSource] || emptyCarrierCounts();
    const postCage = returnEventTotal(shift, cleanSource, 'cage');
    const postPallet = returnEventTotal(shift, cleanSource, 'pallet');
    const cage = Number(pre.cage || 0) + Number(postCage || 0);
    const pallet = Number(pre.pallet || 0) + Number(postPallet || 0);
    return { cage, pallet, total: cage + pallet };
  }

  function removeCustomReturnSource(shift, source) {
    migrateShift(shift);
    const cleanSource = normalizeReturnSourceName(source);
    if (!shift.returnCustomSources.includes(cleanSource)) throw new Error('此站所不是當班特殊回倉站所');
    const totals = customReturnSourceTotals(shift, cleanSource);
    if (totals.total !== 0) {
      throw new Error(`${cleanSource} 尚有回倉紀錄（${totals.pallet}板${totals.cage}籠），請先將00前與00後數量全部歸零後再刪除`);
    }
    delete shift.preMidnightReturns.counts[cleanSource];
    shift.returnCustomSources = shift.returnCustomSources.filter((item) => item !== cleanSource);
    return true;
  }
  ...
  function computeCurrentReturnBucketCounts(shift, timestamp = nowIso()) {
    ...
    const bucket = halfHourBucket(timestamp);
    const bySource = {};
    const sources = [...new Set([...returnSourcesForShift(shift), ...shift.returnEvents.map((event) => event.source)])];
  ...
  function computeCombinedReturnCounts(shift) {
    ...
    const sources = [...new Set([...returnSourcesForShift(shift), ...Object.keys(before.bySource), ...Object.keys(after.bySource)])];
    const bySource = {};
    sources.forEach((source) => {
  ...
  function makeReturnReportText(shift) {
    ...
    const customSources = Array.isArray(shift.returnCustomSources) ? [...shift.returnCustomSources] : [];
    ...
    const sourceOrder = [...RETURN_REPORT_SOURCE_ORDER, ...customSources, ...unknownSources];
    ...
    // 0.064：上半部只列出00後實際有回倉紀錄的來源；0不列出。
    ...
    // 0.064：完整回倉總數比照03:00快速回報，使用全形固定欄寬表格對齊。
    // 下半部將 NS1 / NS2 / NS5 合併為單一「NS」列，板、籠分別加總；其餘站所照固定順序保留。
    // 站所即使為0也必須保留，讓轉運人員可直接逐列核對。
    const returnHeaders = ['站所', '板', '籠'];
    const fixedReturnSummarySources = ['DC2', 'DC4', 'DC6', 'DC9', 'DC11', 'NS', 'TS5', 'SDC', 'DC12', 'DC7', 'CS4'];
    const nsSummary = ['NS1', 'NS2', 'NS5'].reduce((total, source) => {
      const value = combined.bySource[source] || { cage: 0, pallet: 0 };
      total.cage += Number(value.cage || 0);
      total.pallet += Number(value.pallet || 0);
      return total;
    }, { cage: 0, pallet: 0 });
    const lowerUnknownSources = unknownSources.filter((source) => !['NS1', 'NS2', 'NS5'].includes(source));
    const returnSummarySources = [...fixedReturnSummarySources, ...customSources, ...lowerUnknownSources];
    const returnRows = returnSummarySources.map((source) => {
      const value = source === 'NS'
        ? nsSummary
        : (combined.bySource[source] || { cage: 0, pallet: 0 });
      return [
        toFullWidthReportText(source),
        toFullWidthReportText(`${Number(value.pallet || 0)}板`),
        toFullWidthReportText(`${Number(value.cage || 0)}籠`),
      ];
    });
    const returnWidths = returnHeaders.map((header, index) =>
      Math.max(
        reportTextWidth(header),
        ...returnRows.map((row) => reportTextWidth(row[index]))
      )
    );
    const formatReturnRow = (row) =>
      row.map((cell, index) => padReportCell(cell, returnWidths[index]))
        .join('　')
        .trimEnd();

    lines.push(formatReturnRow(returnHeaders));
    returnRows.forEach((row) => lines.push(formatReturnRow(row)));
  ...
  return {
    ...
    uid, nowIso, localDate, naturalStationSort, operationalStationSort, canonicalStationName, displayStationName, stationSeries, stationGroup, stationLabelColorGroup, defaultCarrierForStation, otherCarrier, normalizeCarrier, normalizeReturnSourceName, isValidCustomReturnSourceName, returnSourcesForShift,
    ...
    preMidnightReturnSourceCounts, setPreMidnightReturnCount, adjustPreMidnightReturnCount, computePreMidnightReturnCounts, completePreMidnightReturns, reopenPreMidnightReturns, addCustomReturnSource, customReturnSourceTotals, removeCustomReturnSource,
