(() => {
  ...
  // 0.052：特殊盤點未確認前只存在記憶體草稿；不寫入第二份正式數量。
  ...
  // 0.052：過境貨／回倉採低亮度觸及回饋，只確認「有按到」，不搶夜班＋1的視覺層級。
    ...
    try {
      ...
      requestAnimationFrame(() => {
        if (state.activeView === 'transit' && category === 'transit') {
          flashSoftTouch(main.querySelector(`[data-transit-card="${station}"]`));
        } else {
          flashStationButton(station);
        }
      });
  ...
  // 0.052：特殊盤點是既有中班／夜班／過境／現場資料的高權限確認入口。
  ...
  function specialCarrierTabsHtml(station, selectedCarrier) {
    const defaultCarrier = L.defaultCarrierForStation(station);
    const otherCarrier = L.otherCarrier(defaultCarrier);
    return `<div class="special-carrier-tabs" role="group" aria-label="${stationLabel(station)}載具">
      <button type="button" class="${selectedCarrier === defaultCarrier ? 'active' : ''}" data-special-carrier="${defaultCarrier}" data-station="${station}">${carrierShort(defaultCarrier)}（預設）</button>
      <button type="button" class="${selectedCarrier === otherCarrier ? 'active' : ''}" data-special-carrier="${otherCarrier}" data-station="${station}">${carrierShort(otherCarrier)}（非預設）</button>
    </div>`;
  }
  ...
  function specialStationHtml(shift, station, counts, cageStats, palletStats) {
    ...
    const preview = specialPreview(shift, station, carrier, draft, counts);
    ...
    return `<article class="special-card ${confirmed ? 'is-confirmed' : ''} ${hasDifference ? 'has-difference' : ''}" data-special-card="${station}">
      ...
      ${specialCarrierTabsHtml(station, carrier)}
  ...
  function updateSpecialCardPreview(station) {
    ...
    const preview = specialPreview(shift, station, carrier, draft);
  ...
  function renderSpecial() {
    ...
    main.innerHTML = `${guideDetailsHtml('特殊盤點', '收線最後確認；按確認才寫回正式資料', '中班／夜班／過境／現場都直接讀取既有正式資料，不另外建立第二份數量。輸入先存在本頁草稿，按「確認此站」後才同步修正中班、夜班產出、過境貨與現場盤點。一般提前載走、00前已載走、二次分理不會被本模式重設；應有數仍由來源扣除一般提前載走後計算。')}
      ...
      <div class="special-filter-row">
        <select id="specialGroupFilter" aria-label="特殊盤點系列分類">
          <option value="ALL">全部系列</option>
          ${L.GROUP_ORDER.map((prefix) => `<option value="${prefix}" ${group === prefix ? 'selected' : ''}>${shortGroupTitle(prefix)}</option>`).join('')}
        </select>
        <button id="specialUnconfirmedToggle" type="button" class="filter-toggle-btn ${unconfirmedOnly ? 'active' : ''}" aria-pressed="${unconfirmedOnly}">未確認（${unconfirmedCount}）</button>
      </div>
      <button id="specialDifferenceToggle" type="button" class="filter-toggle-btn special-difference-toggle ${differenceOnly ? 'active' : ''}" aria-pressed="${differenceOnly}">只顯示有差異（${differenceCount}）</button>
      <div>${groupSections || '<div class="empty-state">沒有符合條件的站所。</div>'}</div>`;
  ...
  function renderReturns() {
    ...
    main.querySelectorAll('[data-return-change]').forEach((button) => button.addEventListener('click', async () => {
      try {
        ...
        renderReturns();
        updateHeader();
        requestAnimationFrame(() => flashSoftTouch(main.querySelector(`[data-return-card="${feedbackSource}"]`)));
      } catch (error) {
        showToast(error.message);
      }
    }));
  ...
  function exportAllJSON() { const payload = { app: '物流夜班點貨', schemaVersion: 16, exportedAt: new Date().toISOString(), state }; downloadFile(`夜班點貨_完整備份_${L.localDate()}.json`, JSON.stringify(payload, null, 2), 'application/json'); showToast('JSON備份已匯出'); }
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.052';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.052.js?v=0.052',
        { updateViaCache: 'none' }
      );
