(() => {
  ...
  function preMidnightReturnPanelHtml(shift) {
    ...
    return `<section class="card pre-midnight-return-card ${completed ? 'is-complete' : ''}">
      ...
      ${preMidnightReturnExpanded ? `<div class="pre-midnight-return-body">
        ...
        <div class="pre-midnight-return-list">${[...L.RETURN_REPORT_SOURCE_ORDER, ...(shift.returnCustomSources || []), ...L.RETURN_SOURCES.filter((source) => !L.RETURN_REPORT_SOURCE_ORDER.includes(source))].map((source) => {
          const value = summary.bySource[source] || { cage: 0, pallet: 0 };
          const selected = Number(value[carrier] || 0);
          const isCustom = (shift.returnCustomSources || []).includes(source);
          return `<article class="pre-midnight-return-row ${isCustom ? 'is-custom' : ''}" data-pre-return-row="${source}">
            <strong>${source}${isCustom ? '<small class="return-custom-tag">特殊</small>' : ''}</strong>
  ...
  function renderReturns() {
    ...
    const sources = L.returnSourcesForShift(shift);
    const customSources = new Set(shift.returnCustomSources || []);
    ...
    main.innerHTML = `
      ${guideDetailsHtml('回倉紀錄', '00前回倉＋00後30分鐘批次計數', '00前回倉獨立記錄上大夜前已拆分／回倉的籠板；00後面板只顯示目前30分鐘時段數量，到下一時段會自動歸零，下方歷史時段紀錄仍完整保留。複製回報的完整回倉總數會自動把00前與00後相加；偶發來源可用「＋特殊站所」加入當班。')}
      ...
      <button id="addCustomReturnSourceBtn" type="button" class="secondary-btn full-width return-add-custom-btn">＋ 特殊站所</button>
      <div class="return-source-grid">${sources.map((source) => {
        const value = current.bySource[source] || { cage: 0, pallet: 0, total: 0 };
        const isCustom = customSources.has(source);
        return `<div class="return-source-card ${isCustom ? 'is-custom' : ''}" data-return-card="${source}"><strong>${source}</strong>${isCustom ? `<button type="button" class="return-custom-delete" data-delete-custom-return-source="${source}" aria-label="刪除特殊回倉站所 ${source}">刪除</button>` : ''}<span>${value[carrier]}</span><small>${isCustom ? '特殊站所｜' : ''}本時段：${carrierPair(value.cage, value.pallet)}</small><div><button class="return-minus" data-return-change="-1" data-return-source="${source}">−1</button><button class="return-plus" data-return-change="1" data-return-source="${source}">＋1</button></div></div>`;
      }).join('')}</div>
      <section class="card return-grand-total" aria-label="回倉統計總數">
        <div class="return-grand-total-row"><span>目前時段</span><strong>${current.carrierTotals.total}</strong><small>${carrierPair(current.carrierTotals.cage, current.carrierTotals.pallet)}</small></div>
        <div class="return-grand-total-row"><span>00後累計</span><strong>${returns.carrierTotals.total}</strong><small>${carrierPair(returns.carrierTotals.cage, returns.carrierTotals.pallet)}</small></div>
        <div class="return-grand-total-row is-final"><span>今日含00前</span><strong>${combinedReturns.carrierTotals.total}</strong><small>${carrierPair(combinedReturns.carrierTotals.cage, combinedReturns.carrierTotals.pallet)}</small></div>
      </section>
      ...
      <section class="card return-copy-card"><button id="copyReturnReportBtn" class="primary-btn full-width" type="button">複製回倉回報文字</button><p class="small-note">上半部保留00後各時段紀錄；特殊站所有數量時也會列入。下半部固定欄寬表格列出「00前＋00後」完整回倉總數，當班特殊站所即使為0也保留。</p></section>`;
    ...
    el('addCustomReturnSourceBtn')?.addEventListener('click', async () => {
      const raw = window.prompt('輸入特殊回倉站所，例如 CS3');
      if (raw === null) return;
      try {
        const source = L.addCustomReturnSource(shift, raw);
        await saveState();
        showToast(`已新增特殊回倉站所 ${source}`);
        renderReturns();
      } catch (error) {
    ...
    main.querySelectorAll('[data-delete-custom-return-source]').forEach((button) => button.addEventListener('click', async () => {
      const source = button.dataset.deleteCustomReturnSource;
      if (!window.confirm(`確定刪除特殊回倉站所 ${source}？`)) return;
      try {
        L.removeCustomReturnSource(shift, source);
        await saveState();
        showToast(`${source} 已刪除`);
        renderReturns();
        updateHeader();
      } catch (error) {
        window.alert(error.message);
  ...
  function exportAllJSON() { const payload = { app: '物流夜班點貨', schemaVersion: 18, exportedAt: new Date().toISOString(), state }; downloadFile(`夜班點貨_完整備份_${L.localDate()}.json`, JSON.stringify(payload, null, 2), 'application/json'); showToast('JSON備份已匯出'); }
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.064';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.064.js?v=0.064',
        { updateViaCache: 'none' }
      );
