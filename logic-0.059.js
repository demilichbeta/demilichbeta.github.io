(function (root, factory) {
  ...
  const RETURN_REPORT_SOURCE_ORDER = ['DC2', 'DC4', 'DC6', 'DC9', 'DC11', 'NS1', 'NS2', 'TS5', 'SDC', 'DC12', 'DC7', 'CS4'];
  ...
  function migrateShift(shift) {
    ...
    // 0.059：00前回倉獨立保存，不混入00後30分鐘 returnEvents。
  ...
  function makeReturnReportText(shift) {
    ...
    const lines = sourceOrder.map((source) => {
      ...
      return entries.length ? `${source}｜${entries.join('｜')}` : `${source}｜0`;
    });
