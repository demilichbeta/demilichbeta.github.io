(() => {
  ...
  // 0.055：特殊盤點未確認前只存在記憶體草稿；不寫入第二份正式數量。
  ...
  // 0.055：過境貨／回倉採低亮度觸及回饋，只確認「有按到」，不搶夜班＋1的視覺層級。
  ...
  // 0.055：＋1回饋使用獨立浮動節點；連續點擊可同時存在，位置以向上為主隨機散開。
  ...
  // 0.055：特殊盤點是既有中班／夜班／過境／現場資料的高權限確認入口。
  ...
  function specialStationHtml(shift, station, counts, cageStats, palletStats) {
    ...
    // 0.055：若使用者尚未手動選載具，且只有另一載具有異常，直接打開異常載具，避免問題藏在第二頁籤。
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.055';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.055.js?v=0.055',
        { updateViaCache: 'none' }
      );
