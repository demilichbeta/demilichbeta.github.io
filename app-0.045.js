  ...
  function renderOnline() {
    ...
    main.innerHTML = `
      <section class="online-toolbar" aria-label="二次分理快速操作">
        ...
        <details class="online-inline-guide"><summary><span>轉完成才計數量</span><span>操作說明</span></summary><p>尚未轉完成的數字只代表待處理，不會進入統計核對或快速回報。</p></details>
      </section>
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.045';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.045.js?v=0.045',
        { updateViaCache: 'none' }
      );
