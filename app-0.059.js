  ...
  function preMidnightReturnPanelHtml(shift) {
    ...
    return `<section class="card pre-midnight-return-card ${completed ? 'is-complete' : ''}">
      ...
      ${preMidnightReturnExpanded ? `<div class="pre-midnight-return-body">
        ...
        <div class="pre-midnight-return-list">${L.RETURN_SOURCES.map((source) => {
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.059';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.059.js?v=0.059',
        { updateViaCache: 'none' }
      );
