(() => {
  ...
  // 0.053：特殊盤點未確認前只存在記憶體草稿；不寫入第二份正式數量。
  ...
  // 0.053：過境貨／回倉採低亮度觸及回饋，只確認「有按到」，不搶夜班＋1的視覺層級。
  ...
  // 0.053：特殊盤點是既有中班／夜班／過境／現場資料的高權限確認入口。
  ...
  function specialCarrierTabsHtml(station, selectedCarrier, previews) {
    ...
    const tabHtml = (carrier) => {
      ...
      const defaultMark = isDefault ? '<span class="special-default-mark" aria-label="預設載具" title="預設載具">預</span>' : '';
      const differenceMark = bad ? `<span class="special-carrier-diff">差${preview.difference > 0 ? '+' : ''}${preview.difference}</span>` : '';
      return `<button type="button" class="${active ? 'active' : ''} ${bad ? 'has-difference' : ''}" data-special-carrier="${carrier}" data-station="${station}"><span class="special-carrier-name">${carrierShort(carrier)}${defaultMark}</span>${differenceMark}</button>`;
    };
  ...
  function specialStationHtml(shift, station, counts, cageStats, palletStats) {
    ...
    // 0.053：若使用者尚未手動選載具，且只有非預設載具有異常，直接打開異常載具，避免問題藏在第二頁籤。
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        const reloadKey = 'night-pallet-counter-controller-reload-0.053';
        if (sessionStorage.getItem(reloadKey)) return;
        sessionStorage.setItem(reloadKey, '1');
        window.location.reload();
      });
      const registration = await navigator.serviceWorker.register(
        './service-worker-0.053.js?v=0.053',
        { updateViaCache: 'none' }
      );
